...
# fail2ban - 自动封禁
# OSSEC / Wazuh - HIDS
# ELK Stack - 日志分析`,codeLang:"bash"}]}]}];function p(){const s=r(g),t=r(""),o=r("all"),l=a(()=>{const e=new Set;return s.value.forEach(n=>e.add(n.category)),Array.from(e)}),d=a(()=>{const e=new Set;return s.value.forEach(n=>n.tags.forEach(i=>e.add(i))),Array.from(e)}),S=a(()=>s.value.filter(e=>{const n=!t.value||e.title.toLowerCase().includes(t.value.toLowerCase())||e.excerpt.toLowerCase().includes(t.value.toLowerCase())||e.tags.some(c=>c.toLowerCase().includes(t.value.toLowerCase()))||e.author.toLowerCase().includes(t.value.toLowerCase()),i=o.value==="all"||e.category===o.value;return n&&i}));return{allTutorials:s,searchQuery:t,categoryFilter:o,allCategories:l,allTags:d,filteredTutorials:S,getTutorialById:e=>s.value.find(n=>n.id===e),levelLabel:e=>({beginner:"入门",intermediate:"中级",advanced:"高级",expert:"专家"})[e]}}export{p as u};
